...
-- list comprehension style for bind; not right yet, not compositional
{-ppExp e@(Bind s (Lam t f)) used = x ++ " <- " ++ (ppExp s used) ++ "; \n" ++ (ppExp (f (Var x t)) used')
    where x = freshName e used
          used' = x : used
          -}

ppExp (Bind s f) used = (ppExp s used) ++ " >>= (" ++ (ppExp f used) ++ ")"
...
ling = Const "ling" (SetRepr ERepr)
phil = Const "phil" (SetRepr ERepr)
--a_rel = SetLit $ map (\i -> Const i ERepr) ["alice", "bob", "dave"]
dies = Const "dies" (ERepr ==> TRepr)
met = Const "met" (ERepr ==> ERepr ==> TRepr)
sif = 
    Lam (SetRepr TRepr) $ \m ->
        Lam (SetRepr TRepr) $ \n ->
...
sent3 :: Exp (Set (Set T))
sent3 = Bind ling $ Lam ERepr $ \x -> SetLit [Bind phil $ Lam ERepr $ \y -> SetLit [App (App met y) x]]
